...
class HetAgg(nn.Module):
	...
	def a_content_agg(self, id_batch): #heterogeneous content aggregation
		...
		a_net_embed_batch = self.feature_list[4][id_batch]
	...
	def p_content_agg(self, id_batch):
		...
		p_v_net_embed_batch = self.feature_list[0][id_batch]
		p_a_net_embed_batch = self.feature_list[1][id_batch]
	...
	def v_content_agg(self, id_batch):
		...
		v_net_embed_batch = self.feature_list[5][id_batch]
	...
	def t_content_agg(self, id_batch): #heterogeneous content aggregation
		embed_d = self.embed_d
	
		t_net_embed_batch = self.feature_list[6][id_batch]

		concate_embed = t_net_embed_batch.view(len(id_batch[0]), 1, embed_d)
	...
	def node_neigh_agg(self, id_batch, node_type): #type based neighbor aggregation with rnn 
		...
		if node_type == 1 or node_type == 2 or node_type == 4:
	...
	def node_het_agg(self, id_batch, node_type): #heterogeneous neighbor aggregation
		...
		for i in range(len(id_batch)):
			if node_type == 1:
				...
				t_neigh_batch[i] = self.a_neigh_list_train[3][id_batch[i]]
			elif node_type == 2:
				...
				t_neigh_batch[i] = self.p_neigh_list_train[3][id_batch[i]]
			elif node_type == 4:
				a_neigh_batch[i] = self.t_neigh_list_train[0][id_batch[i]]
				p_neigh_batch[i] = self.t_neigh_list_train[1][id_batch[i]]
				v_neigh_batch[i] = self.t_neigh_list_train[2][id_batch[i]]
				t_neigh_batch[i] = self.t_neigh_list_train[3][id_batch[1]]
			else:
				...
				t_neigh_batch[i] = self.v_neigh_list_train[3][id_batch[i]]
		...
		concate_embed = torch.cat((c_agg_batch_2, a_agg_batch_2, p_agg_batch_2,\
		 v_agg_batch_2, t_agg_batch_2), 1).view(len(c_agg_batch), 5, self.embed_d * 2)
		if node_type == 1:
			...
			atten_w = self.act(torch.bmm(concate_embed, self.t_neigh_att.unsqueeze(0).expand(len(c_agg_batch),\
			 *self.t_neigh_att.size())))
		else:
		...
		concate_embed = torch.cat((c_agg_batch, a_agg_batch, p_agg_batch,\
		 v_agg_batch, t_agg_batch), 1).view(len(c_agg_batch), 5, self.embed_d)
	...
	def het_agg(self, triple_index, c_id_batch, pos_id_batch, neg_id_batch):
		...
		if triple_index == 0:
			...
			return [], [], []
			'''
		#12 cases for academic data (author, paper, venue, topic)
		if triple_index == 0:
			c_agg = self.node_het_agg(c_id_batch, 1)
			p_agg = self.node_het_agg(pos_id_batch, 1)
			n_agg = self.node_het_agg(neg_id_batch, 1)
